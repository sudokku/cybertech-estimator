...
**Free models — what to expect.** Tested live on OpenRouter's free tier: only a few free models support structured outputs (the settings page marks the others "no structured output" and prefers a capable one when suggesting); when a model cannot take a strict schema the provider retries once in prompt-only JSON mode. Free models are reasoning models, so the plugin disables reasoning where the model allows it (otherwise the whole token budget is spent thinking and the reply is empty). Upstream rate limits (HTTP 429 with a short `Retry-After`) get one bounded retry. Small models occasionally slip a bracket; the validator repairs trailing commas and mismatched closers before judging the reply. In a 10-run batch (two free models × five scenarios incl. a prompt-injection attempt) every accepted reply honoured the week count, used only the listed roles and contained no money; the rest fell back to the built-in narrative.
